...
class AdaptiveRANSAC(RANSAC):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            goal_inliers_ratio=goal_inliers_ratio)
        self.ada_thr = None
    ...
    def run(
        ...
        if threshold is None:
            median_y, _ = torch.median(data_y, dim=1, keepdims=True)
            abs_deviation = torch.abs(data_y - median_y)

            # The residual threshold is the median of the absolute deviation.
            self.ada_thr, _ = torch.median(abs_deviation, dim=1, keepdims=True)
        else:
            self.ada_thr = threshold

        # best_model = super().run(data)
        best_model = do_faster_ransac(
            data,
            max_iterations=self.max_iterations,
            plane_distance_thr=self.ada_thr)
