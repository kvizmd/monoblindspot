...
class OracleScaleOxtsOGMIntegrator(OGMIntegrator):
    ...
    def update_occupancy(
        ...
        T_cam2global = torch.linalg.solve(
            inputs['T_imu2global', 0], inputs['T_imu2global', frame_idx])
        T_cam2cam = torch.bmm(
            inputs['T_imu2cam'], torch.bmm(T_cam2global, inputs['T_cam2imu']))
        T_cam2cam = torch.bmm(T_cam2cam, T_scale)
        ...
        # Noise in KITTI GPS/IMU could cause strange poses and nan.
        src_ogm.data = torch.nan_to_num(src_ogm.data)
